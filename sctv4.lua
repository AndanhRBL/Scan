...
local function generateLua()
    ...
    local lines = {
        ...
        "-- Generated by Roblox UI Scanner V5",
...
local function exportFolder()
    ...
    -- Xuất Remotes bắt được thành Code Lua để AI đọc

    local remoteLines = {

        "-- Captured Remotes by UI Scanner V5 Spy",

        "-- Dùng cho AI để viết Auto Farm / Auto Quest:",
    ...
    for _, log in ipairs(remoteLogs) do
        ...
        table.insert(remoteLines, string.format("-- [%s] %s -> %s", os.date("%X", log.time), log.type, log.remote))

        table.insert(remoteLines, log.code)

        table.insert(remoteLines, "")

    end

    writeTextFile(data .. "/remotes_captured.lua", table.concat(remoteLines, "\n"))
