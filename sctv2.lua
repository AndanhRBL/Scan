--[[
    ROBLOX UI SCANNER V3 + LIVE DISCOVERY + UI-to-LUA GENERATOR
    Executor-compatible client-side tool

    Features:
      - Scan PlayerGui + relevant CoreGui + Workspace BillboardGui/SurfaceGui
      - Include hidden UI that already exists
      - WATCH CHANGES: debounced UI added/removed discovery
      - Search
      - Explorer-like tree
      - Property inspector
      - Copy selected JSON / all JSON
      - Export folder for VS Code
      - Persistent discovery history
      - UI-to-Lua generator

    Scope:
      Client-visible UI/Instance metadata only.
      Does NOT decompile or extract server source code.
]]
...
new("TextLabel", {
    ...
    Text = "Live UI discovery / Explorer",
    TextColor3 = MUTED,
    TextSize = 11,
    TextXAlignment = Enum.TextXAlignment.Left
}, top)
...
local function generateLua()
    ...
    local lines = {
        "-- Generated by Roblox UI Scanner V3",
...
local function collectUI()
    ...
    local function walk(obj, depth)
        if seen[obj] or shouldSkip(obj) then return end
        seen[obj] = true
        ...
        for _, child in ipairs(children) do
            if isUIObject(child) then
                walk(child, depth + 1)
            elseif scanScope == "all" and (child:IsA("Model") or child:IsA("Folder")) then
                -- Only descend through non-UI containers in Workspace when looking
                -- for BillboardGui / SurfaceGui descendants.
                for _, nested in ipairs(child:GetChildren()) do
                    if isUIObject(nested) then
                        walk(nested, depth + 1)
                    elseif nested:IsA("Model") or nested:IsA("Folder") then
                        -- handled naturally by the outer recursive helper below
                    end
                end
            end
        end
...
local function attachWatch()
    ...
    local function refreshWatch()
        ...
        task.delay(WATCH_DEBOUNCE, function()
            ...
            rebuildTree()
            statusLabel.Text = "Watching | " .. #allRecords .. " live | " .. #discoveredOrder .. " discovered"
        end)
    end
...
local function scan()
    ...
    statusLabel.Text =
        ...
        (scanScope == "all" and "ALL" or "PLAYERGUI")
...
local function exportFolder()
    if not fsAvailable() then
        ...
        warn("[UI Scanner] Need makefolder + writefile.")
        return
    end
    ...
    local root = "UI_Scanner_V3_Export"
    ...
    local readme = [[# Roblox UI Scanner Export
...
- `data/objects/` - individual object JSON files
- `docs/tree.txt` - readable hierarchy

The scanner records client-visible UI/Instance metadata.
It does not export server source code or decompile scripts.
]]
    ...
    statusLabel.Text =
        "Exported " .. written ..
        " live files + " .. #discoveredOrder .. " discovered + Lua"
...
search:GetPropertyChangedSignal("Text"):Connect(rebuildTree)
...
print("[UI Scanner v2] Loaded. Scan + Debounced Watch + Export ready.")
