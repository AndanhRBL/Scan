--[[
    ROBLOX UI SCANNER V4 + SAFE BATCHED DISCOVERY + UI-to-LUA GENERATOR
...
local function generateLua()
    ...
    local lines = {
        "-- Generated by Roblox UI Scanner V4",
...
local function scan()
    ...
    statusLabel.Text =
        ...
        (truncated and " | LIMIT " .. MAX_SCAN_OBJECTS or "")
...
local function exportFolder()
    ...
    local root = "UI_Scanner_V4_Export"
...
It does not export server source code or decompile scripts.
    ...
    for i, record in ipairs(allRecords) do
        ...
        if i % EXPORT_YIELD_EVERY == 0 then
            statusLabel.Text = "Exporting object files... " .. i .. "/" .. #allRecords
            task.wait()
        end
    end
...
print("[UI Scanner v4] Loaded. Batched Scan + Debounced Watch + Export ready.")
